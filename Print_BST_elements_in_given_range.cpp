...
void printNearNodesUtil(Node *root, int low, int high, vector<int> &rV) {

    if (!root)
        return;

    if (root -> data > low)
        printNearNodesUtil(root -> left, low, high, rV);

    if (root -> data >= low && root -> data <= high)
        rV.emplace_back(root -> data);

    if (root -> data < high)
        printNearNodesUtil(root -> right, low, high, rV);
}

vector<int> printNearNodes(Node *root, int low, int high) {
    vector<int> rangeValues;
    printNearNodesUtil(root, low, high, rangeValues);
    return rangeValues;
}
